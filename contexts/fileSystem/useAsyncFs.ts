...
import { configure, CopyOnWriteFS, mounts, Stats } from "@zenfs/core";
...
import {
  ...
  type RootFileSystem,
} from "contexts/fileSystem/zenfs";
import {
  ICON_CACHE,
  ICON_CACHE_EXTENSION,
  SESSION_FILE,
} from "utils/constants";
import { UNKNOWN_STATE_CODES } from "contexts/fileSystem/core";
...
const useAsyncFs = (): AsyncFSModule => {
  ...
  const asyncFs: AsyncFS = useMemo(
    () => ({
      ...
      mkdir: (path, overwrite = false) =>
        new Promise((resolve, reject) => {
          fs?.mkdir(path, { recursive: overwrite }, (error) =>
                ...
                try {
                  if (path !== SESSION_FILE) {
                    ...
                    fs?.exists(
                      cachedIconPath,
                      (exists) =>
                        exists && fs?.unlink(cachedIconPath, () => false)
                    );
                  }
                } catch {
  ...
  useEffect(() => {
    ...
    const setupFs = async (): Promise<void> => {
      try {
        await configure(FileSystemConfig());

        const loadedRootFs = mounts.get("/");

        if (!(loadedRootFs instanceof CopyOnWriteFS)) {
          throw new TypeError("ZenFS root is not configured as CopyOnWriteFS.");
        }

        const loadedFs = createFSModule();
        ...
        setRootFs(
          createRootFileSystem(
            loadedRootFs,
            loadedRootFs.readable,
            loadedRootFs.writable
          )
        );
        runQueuedFsCalls(loadedFs);
      } catch (error) {
